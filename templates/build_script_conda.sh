...
wget https://repo.anaconda.com/miniconda/Miniconda3-py311_23.10.0-1-Linux-s390x.sh -O miniconda.sh 
...
# Check if the tar.bz2 file exists in linux-s390x directory
linux_file="/root/miniconda/envs/$PACKAGE_NAME/conda-bld/linux-s390x/*.tar.bz2"
if ls $linux_file 1> /dev/null 2>&1; then
   binary_path=$(ls $linux_file)
   echo "Found $PACKAGE_NAME.tar.bz2 in linux-s390x directory."
else
   echo "$PACKAGE_NAME.tar.bz2 not found in linux-s390x directory. Searching in noarch directory..."
   ...
   if [ -n "$found_file" ]; then
       ...
       echo "Error: $PACKAGE_NAME.tar.bz2 file not found in linux-s390x or noarch directory."
       exit 3
   fi
fi
